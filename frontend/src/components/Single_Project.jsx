...
const Single_Project = ({ image, title, description, liveLink, githubLink }) => {
    ...
    const single_project = useRef(null);

    useEffect(() => {
        const ctx = gsap.context(() => {

            gsap.from(".individual_project_object", {
                y: 45,
                opacity: 0,
                duration: 0.75,
                ease: "power2.out",
                scrollTrigger: {
                    trigger: ".individual_project_object",
                    start: "top 80%",
                },
                clearProps: true
            });
    ...
    return (
        <div className='rounded-xl border-2 border-[#18083d] hover:scale-102 hover:shadow-purple-950 hover:shadow-lg transition duration-300' ref={single_project}>
            <div className='individual_project_object'>
